...
notesRouter.post('/',async (request,response)=>{
     // get the new notes from the request body
     ...
     // if the token missing or invalid , return an error
     if(!token || !decodedToken.id){
        return response.status(401).json({
            error:'token missing or invalid'
        });
     } 
    // if the token is valid, find the document in the user collection  that matches the decodedToken id value
